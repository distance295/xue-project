 $(function(){
    ...
    // var trim_Version=version[1].replace(/[ ]/g,"");
       ...
       if(isFirefox=navigator.userAgent.indexOf("Firefox")>0){  
        ...
        $('.section-1 .title').css({animation:'bounceInDown 1s',opacity:1})    
        ...
        var iHeight=0
        $('.section-5 .item').each(function(){
            iHeight+=$(this).height()
        })
        if(iHeight<$('body').height()*0.5){
            $('.shadow').hide()
        }

        $('.section-5 .shadow').css({top:$('.section-5 .title').height()+$('body').height()*0.1+190+$('.section-5 .wrong').height()})
        if(bodyWidth<=768){
            $('.section-5 .shadow').css({top:$('.section-5 .title').height()+$('body').height()*0.1+165+$('.section-5 .wrong').height()})
        }
        if(json.process==1){
            drawPie(1);
            $('.section-2 .process').hide().eq(json.process-1).show().css({right:"20%"}).find('.time').html('6月7日 9:30开始')
        }
        else if(json.process==2){
            drawPie();
            $('.section-2 .process').eq(2).hide()
            $('.section-2 .process').eq(0).css({right:"50%",marginRight:-$('.section-2 .process').eq(0).width()/2}).find('.ball').addClass('active')
            $('.section-2 .process').eq(1).css({right:'10%'}).find('.time').html('未提交')
            if(bodyWidth<=360){
                $('.section-2 .process').eq(1).css({right:'5%'})
            }
        }
            ...
            $('.section-2 .process').eq(0).css({right:"60%"})
            $('.section-2 .process').eq(1).css({right:"50%",marginRight:-$('.section-2 .process').eq(1).width()/2}).find('.ball').addClass('active')
            $('.section-2 .process').eq(2).css({right:'10%'}).find('.time').html('未订正')

        }
        else if(json.process==4){
            drawPie();
            $('.section-2 .process').eq(0).hide()
            $('.section-2 .process').eq(1).css({right:'60%',top:7})
            $('.section-2 .process').eq(2).css({right:"50%",marginRight:-$('.section-2 .process').eq(2).width()/2}).find('.ball').addClass('active')
        }
        ...
        $('.section-6 img,.section-7 img').css({height:$('body').height()*0.23})

         /* 图片点击放大效果 */
        $('.section-6 img ').on('click',function(){
            $(this).clone().css({height:$('body').height()*0.8,marginTop:-$('body').height()*0.4}).appendTo($('.section-6 .cover')).closest('.cover').fadeIn().on('click',function(){
                $('.section-6 .cover img').remove()
                $(this).closest(".cover").fadeOut()
            })
        })
        $('.section-7 img').on('click',function(){
            $(this).clone().css({height:$('body').height()*0.8,marginTop:-$('body').height()*0.4}).appendTo($('.section-7 .cover')).closest('.cover').fadeIn().on('click',function(){
        ...
        function tabClick(){
            $('.section-3 .item').each(function(index){
                if(isFirefox=navigator.userAgent.indexOf("Firefox")>0){  
                    ...
                    $(this).css({top:40*(4-l+index),zIndex:index})
                }
            })
                ...
                if(index%2==0){
                    if(isFirefox=navigator.userAgent.indexOf("Firefox")>0){  
                       $(this).css({transform:'translate('+(-bodyWidth)+'px,'+(40*(4-l+index))+'px)'+'  rotateX(-30deg)'}) 
                        ...
                        $(this).css({top:40*(4-l+index),zIndex:index,transform:'translate('+(-bodyWidth)+'px,'+'0px)'+'  rotateX(-30deg)'})
                    }
                }
                else{
        ...
        function tabIn(){
            $('.section-3 .item').each(function(index){
                    if(isFirefox=navigator.userAgent.indexOf("Firefox")>0){  
                        ...
                        $(this).css({top:40*(4-l+index),zIndex:index,transform:'translate(0px,'+'0px)'+'rotateX(-30deg)'})  
                    }
            })   
        }
