<!--学生个人主页tab标签开始-->
   <div class="homepage-public-tab">
   	   <ul>
          <li class="current forGuyTab"><a href="javascript:void(0)">新鲜事</a></li>
          <li class="forGuyTab"><a href="javascript:void(0)">晒成绩</a></li>
          <li class="forGuyTab"><a href="javascript:void(0)">关注的人</a></li>
       </ul>
   </div>
<!--个人主页tab标签结束-->
<div class="row bottom m20">
    <div class="col-md-9 top m20 fresh-main-wrapper">
<!--新鲜事开始-->
        <div class="focusGuyTab">
            <div class="all-fresh-list">
                <link rel="import" href="../../widget/Public.Dynamic/dynMy.tpl?__inline">
            </div>
        </div>
<!--新鲜事结束-->
   
<!--晒成绩开始-->
       <div class="center-teacher-report focusGuyTab clearfix hide">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
          <link rel="import" href="../../widget/Public.Module/glory-modal.tpl?__inline">
        </div>
<!--晒成绩结束-->
  
<!--关注的人开始-->
       <div class="focusGuyTab hide">
           <link rel="import" href="../../widget/Public.Dynamic/dynAttention.tpl?__inline">
       </div>
<!--关注的人结束-->
    </div>
    <div class="col-md-3 top m20">
        <div class="center-left-m">
            <link rel="import" href="center-stu-info.tpl?__inline">
            <link rel="import" href="stuVerify.tpl?__inline">
            <link rel="import" href="center-visit.tpl?__inline">
        </div>
    </div>
</div>